...
export function Navbar() {
  ...
  return (
    <nav
      className={`absolute md:fixed top-0 left-0 right-0 z-[40] transition-all duration-300 pointer-events-none md:pointer-events-auto ${
        isScrolled ? "bg-transparent md:bg-white md:shadow-md" : "bg-transparent md:bg-white/95"
      }`}
    >
      <div className="max-w-7xl mx-auto px-8">
        <div className="flex justify-between items-center h-20">
          {/* Left: CTA Button */}
          <div className="hidden md:flex items-end gap-3 justify-end">
           <img 
              src="/logo.png" 
          ...
          <div className="hidden md:flex items-center gap-8 justify-end">
            {reversedNavLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`transition-colors text-base leading-6 ${
          ...
          <button
            ...
            className={`md:hidden fixed top-5 left-5 z-[60] p-2.5 rounded-xl transition-all shadow-md border pointer-events-auto ${
              isScrolled || isOpen 
